...
import {parseCodeSource,parseTableSource,parseImageSource} from './complex-source.js';
...
export function parse(markdown, assetPrefix='folio-asset://unavailable/', editing=false) {
 ...
 md.core.ruler.push('folio-headings', state=>{
  for(let i=0;i<state.tokens.length;i++) {
   ...
   if(t.type==='heading_open') {
    ...
    let id=slug;
    if(used.has(id)){
     let n=nextHeadingSuffix.get(slug)||1;
     do{id=slug+'-'+n++;}while(used.has(id));
     nextHeadingSuffix.set(slug,n);
    }
    used.add(id);
 ...
 const t=tokens[idx], ref=t.attrGet('src')||'', id=images.length;
  ...
  let assetRef=ref;try{assetRef=decodeURIComponent(ref);}catch{}
  return `<figure class="image-frame"${t.meta?.complexId ? ` data-complex="${t.meta.complexId}"` : ''}><img loading="lazy" data-image="${id}" src="${assetPrefix}${encodeURIComponent(assetRef)}" alt="${alt}"><figcaption class="image-fallback" hidden>Image unavailable · ${alt}. Use “Allow image folder…” for local images.</figcaption></figure>`;
 };
 for(const rule of ['fence','code_block']) md.renderer.rules[rule]=(tokens,idx)=>{
  ...
  return `${preview}<section class="code-block"${t.meta?.complexId ? ` data-complex="${t.meta.complexId}"` : ''}><button class="code-copy" type="button" data-copy="${id}" aria-label="Copy code block">Copy</button><details class="source-details" open><summary><span>${md.utils.escapeHtml(language||'Text')}${fallback}</span></summary><pre tabindex="0" aria-label="Code"><code>${rendered}</code></pre></details></section>`;
 };
 const originalTableOpen=md.renderer.rules.table_open;
 md.renderer.rules.table_open=(tokens,idx,options,env,self)=>`<div class="table-scroll" tabindex="0" role="region" aria-label="Scrollable table"${tokens[idx].meta?.complexId ? ` data-complex="${tokens[idx].meta.complexId}"` : ''}>`+(originalTableOpen?originalTableOpen(tokens,idx,options,env,self):self.renderToken(tokens,idx,options));
 ...
 if(editing){
  ...
  const claim=(token,kind,start,finish)=>{
   const id='c'+complexBlocks.length;complexBlocks.push({id,kind,start,end:finish});
   token.meta={...token.meta,complexId:id};
  };
  for(let i=0;i<tokens.length;i++){
   const t=tokens[i];
   if(t.level!==0||!t.map)continue;
   const start=base+offsets[t.map[0]],finish=base+(offsets[t.map[1]]??markdown.length),raw=originalSource.slice(start,finish);
   if(t.type==='fence'&&parseCodeSource(raw)&&!['math','latex','mermaid'].includes((t.info||'').trim().toLowerCase()))claim(t,'code',start,finish);
   if(t.type==='table_open'&&parseTableSource(raw))claim(t,'table',start,finish);
   if(t.type==='paragraph_open'){
    const inline=tokens[i+1],child=inline?.children;
    if(child?.length===1&&child[0].type==='image'&&parseImageSource(raw)){
     claim(child[0],'image',start,finish);t.hidden=true;
